...
if(Array.isArray(window.CHAPTERS)&&window.CHAPTERS.length){
 applyFullChapters(window.CHAPTERS.slice(0,40));
}else{
 fetch('manuscrito.md').then(response=>{if(!response.ok)throw new Error('No se pudo cargar el manuscrito');return response.text()}).then(markdown=>applyFullChapters(manuscriptChapters(markdown).slice(0,40))).catch(error=>console.warn(error.message));
}

const publicationStatus=document.querySelector('footer small');
if(publicationStatus)publicationStatus.textContent='LIBRO EN DESARROLLO · 40 DE 100 CAPÍTULOS DISPONIBLES';
